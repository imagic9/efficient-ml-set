...
- [x] On `gx10`, run full frozen cis-test and trans-test through the exact C++/ORT
      model/policy/runtime artifacts; save frame and sequence-aware metrics.
- [x] Run full M0-vs-optimized Pi benchmark, at least 1,000 frames, three separate
      processes/repetitions as specified.
- [x] Run the fixed Pi parity subset for **both M0-FP32 and the selected winner**
      and match decisions to the frozen gx10 reference; full test transfer to Pi is
      optional, not required. Record score deltas, not only decision agreement.
- [x] **If the parity subset disagrees, stop before claiming target equivalence.**
      *(It did not — bit-identical, see below.)*
- [x] Capture raw per-frame predictions/timings/system logs.
- [x] Copy artifacts off the Pi immediately.

**DONE 2026-07-20 — F4 complete.** Write-up `results/f4/F4_ANALYSIS.md`; consolidated
`results/f4/f4_summary.json` (all numbers generated by `scripts/summarize_f4.py`). Drivers:
`deploy/pi/run_f4_pi_benchmark.sh`, `scripts/run_f4_frozen_test.sh`, `scripts/eval_frozen_test.py`,
`scripts/compare_pi_parity.py`.

- **Pi latency (real CM5, frozen threads=1, 3 reps ×≥1000 iters):** M0 FP32 **49.06 ms / 20.4 FPS**
  → M2 (final) **21.61 ms / 46.3 FPS = 2.27×**, 3.5× smaller; M4 17.74 ms / 56.4 FPS (2.77×, not
  selected). Tight reps (M2 ±0.36 ms), no throttling.
- **Frozen full-test (test opened ONCE, post-freeze, frozen threshold, §5.4):** on `cis_test`
  (15,827 fr / 897 bobcat) **M2 beats the baseline** — F2 0.6387 vs 0.5812, event-capture 0.8575
  vs 0.7668 at equal 5.5 % false-fire; on `trans_test` (23,275 / 2,338) both drop to ~35–40 %
  event-capture (domain-shift limitation, stated), M2 lower recall but 2.15 % false-fire vs M0's
  4.09 %. cis false-fire 5.5 % is just over the 5 % validation-calibrated budget (held-out
  transfer, reported as-is); status stays `recall_floor_infeasible` for both.
- **Pi↔gx10 parity — BIT-IDENTICAL for M0 AND M2** (max score Δ 0.0, 0 decision disagreements on
  the 47-frame slice). Even the FP32 baseline did not drift (ORT MLAS kernels identical across the
  X925 SVE2/i8mm and A76 NEON paths, as the E6 QEMU rehearsal predicted). The gx10 test accuracy
  transfers to the Pi with zero decision drift — **target equivalence holds**.
